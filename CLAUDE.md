...
The deck fetch always loads `prevLikedIds` from the DB so liked cards never reappear — **except demo accounts**, for which the recycle pass drops the liked-card filter so the deck loops the full pool forever (shared-demo testers never hit an empty "ดูครบแล้ว" deck; the pool is reshuffled each recycle). A demo login is any email in `DEMO_EMAILS` (module-level in `swipe/page.tsx`, sourced from `NEXT_PUBLIC_DEMO_EMAIL`, comma-separated, falling back to `demo@pawmate.app,demo-full@pawmate.app` so no extra env is required). `skippedIds` (session `useRef`) are cleared on recycle so skipped cards cycle back. `likedIds` are never cleared — they mirror the DB. A demo counter (`swipeCount` + `forceNext` refs) triggers `create_demo_match` RPC after 3–7 swipes without a natural match.
