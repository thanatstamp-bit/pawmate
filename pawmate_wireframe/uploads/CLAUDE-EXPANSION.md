...
- DO NOT edit supabase/migrations/001_init.sql. Every phase that touches the DB
  writes a NEW migration file (002_trust.sql, 003_hospitals.sql, ...).
- Existing tables may only be ALTERed to add columns when a phase says so —
  never rename or drop existing columns.
- Navigation update (done once, in Phase 7): bottom nav becomes 4 tabs —
  ปัดการ์ด (swipe), แมตช์ (matches), ดูแล (care), โปรไฟล์ (profile).
...
-- Phase 11, optional (007_vet_bookings.sql)
...
1. Update the bottom nav to 4 tabs: ปัดการ์ด, แมตช์, ดูแล (lucide HeartPulse
   icon), โปรไฟล์. Do not restyle the existing tabs.
...
3. Migration 003_hospitals.sql: hospitals table + read-only RLS.
...
1. Migration 004_lost_pets.sql: lost_pets + lost_pet_sightings with RLS,
...
1. Migration 005_blood.sql: blood_donors, blood_requests, blood_responses with
...
1. Migration 006_health.sql: health_records with owner-only RLS.
...
1. Migration 007_vet_bookings.sql: vet_bookings with owner-only RLS.
