...
> อัปเดตล่าสุด: 2026-06-28 (Session 34 — seed pool ครอบคลุมตัวกรอง swipe เต็มทุกมิติ ~600 pets)
>
...
scripts/
  seed.ts            — ~600 pets (≈300 dogs + ≈300 cats) ครอบคลุมทุก breed/tag/จังหวัด (77) สำหรับตัวกรอง swipe + breeding-compatible cohort ต่อ pet จริง + pre-likes; ลบ seed-bot batch เก่าก่อน reseed อัตโนมัติ
...
**Session 34 (06-28) — Seed pool ครอบคลุมตัวกรอง swipe เต็มทุกมิติ (~600 pets) — push `main`**
ปัญหา: หน้า `/app/swipe` กดตัวกรองหลายเงื่อนไขพร้อมกันแล้ว deck ว่าง ("ดูครบแล้ว!"). **Root cause = domain drift:** `FilterSheet.tsx` ดึงตัวเลือกจาก `lib/data/*` (breeds 21 หมา/15 แมว · 15 tags · 77 จังหวัด) แต่ `scripts/seed.ts` hardcode ลิสต์ของตัวเองที่แคบกว่า → ค่าตัวกรองจำนวนมาก match 0 ตัว: 3 tags (`สงบเงียบ`/`ชอบออกกำลัง`/`เป็นมิตรกับสัตว์อื่น`) ไม่เคยถูก assign, 10 breed หมา + 6 breed แมว ไม่มีในพูล (ทำให้ size filter ขาดด้วย เพราะ size มาจาก `BREED_SIZE_MAP`), จังหวัดมีแค่ 10/77. แถม pool เดิม 85 ตัวแบบสุ่มล้วน → คอมโบ 2–3 ตัวกรองก็แห้ง.
**แก้ (ไฟล์เดียว `scripts/seed.ts` rewrite):** (1) **import จาก `lib/data/*` โดยตรง** (`DOG_BREEDS`/`CAT_BREEDS`, `PERSONALITY_TAGS`, `PROVINCES` — relative path เพราะ `scripts/tsconfig.json` ไม่มี `@/` alias; ไฟล์เหล่านี้เป็น const array ล้วน ts-node คอมไพล์ได้) → single source of truth กัน drift ถาวร. (2) **cleanup pass** ก่อน seed: `auth.admin.listUsers` (paginated) → ลบทุก user ที่ email ขึ้นต้น `seed-bot-` ลงท้าย `@pawmate.internal` (FK cascade `auth.users→profiles→pets→likes/matches/messages` ยืนยันใน `001_init.sql`) → deck ไม่สะสมเวลา re-run; **ไม่แตะ** `demo-deck@pawmate.internal` (ของ 017) หรือบัญชีจริง (prefix match การันตี). (3) **generation แบบ round-robin** (ไม่สุ่มล้วน) ผ่าน `generateCohort(species)`: province queue = ทุกจังหวัด ×2 (154) + major provinces แบบถ่วงน้ำหนัก (146) = 300/species; breed = `breeds[i % len]` (ทุก breed ~14–21 ตัว → กระจาย size อัตโนมัติ); tag primary = `PERSONALITY_TAGS[i % 15]` + extra 1–3 สุ่ม (ทุก tag เป็น primary ~20 ตัว/species); สลับเพศ male/female (breeding มีคู่ครบ); อายุ 1–10 ปีวนครบ; vaccinated ~70% / neutered ~50%; modes ส่วนใหญ่ทั้งสอง. (4) คงบล็อก breeding-cohort + pre-likes → demo เดิม.
**Follow-up (โหมดหาคู่/breeding):** หลัง deploy ทดสอบพบ deck ว่างในโหมด **หาคู่** เพราะ breeding lock candidate ไว้ที่ breed เดียวกัน+เพศตรงข้ามของ pet ตัวเอง → (a) **size filter เป็นกับดัก**: pet active = โกลเด้น (large) แต่เลือก ขนาด=กลาง → candidate ทุกตัว breed เดียวกัน=large → กรองหมด=0. แก้: ซ่อน size selector ในโหมด breeding (`FilterSheet.tsx`, แบบเดียวกับ breed) + swipe page ไม่ apply `filters.size` เมื่อ breeding และ reset size ทิ้งตอนสลับเข้าโหมด breeding. (b) **province ใน breeding บาง**: breed-lock ทำให้ general pool ลง breed นั้นแค่ ~22/77 จังหวัด. แก้ใน `seed.ts`: เปลี่ยน breeding cohort ต่อ real/demo pet จาก "8 ตัวสุ่มจังหวัด" → "1 ตัว/ทุกจังหวัด (77)" เพศตรงข้าม breed เดียวกัน (~80% vaccinated, อายุ/นิสัยหลากหลาย, pre-like แค่ 4 ตัวแรกกัน instant-match ท่วม). ผลหลัง reseed: golden-female-breeding ครบ **77/77 จังหวัด**, scenario เดิม (breeding+นครนายก+vaccinated) → เจอ.
**Follow-up #2 (playdate province+breed ก็ว่าง):** ทดสอบต่อพบว่าแค่เลือก **จังหวัด + สายพันธุ์** ในโหมด playdate ก็ไม่เจอ. วัดจริง: province×breed cells มีข้อมูลแค่ **27%** (443/1617 หมา, 309/1155 แมว). Root cause: general cohort เดิม assign breed (`i%21`) กับ province (จาก queue) ด้วยตัวนับแยกกัน → แต่ละจังหวัดได้แค่ ~2 breed → 73% ของคู่ province+breed ว่าง. **แก้ `seed.ts`:** เปลี่ยน generation เป็น **grid เต็ม province × breed** (วน 77 จังหวัด × ทุก breed × K ตัว/ช่อง, `K_PER_CELL=2`) — n ตัวนับเดียววนแจก tag(primary)/อายุ/เพศ(สลับ → แต่ละช่องมี 2 เพศ → breeding ได้คู่ทุกจังหวัดด้วย), แต่ละช่อง k=0 vaccinated เสมอ (province+breed+vaccinated ไม่ว่าง), tag ใจกว้าง 4–6 ตัว. **batched insert** (chunk 500) → เร็วแม้หลายพันแถว. ลด dedicated breeding cohort เหลือ 6 ตัว/pet (เฉพาะ pre-like instant-match เพราะ grid คุม province coverage แล้ว). **เลือกผ่าน AskUserQuestion: K=2 (~5,600)** — การันตี single filter ทุกตัว + 2-way combo ทุกคู่ (รวม province+breed); 3-way (province+breed+tag เจาะจง) ได้บ้าง (จำกัดเชิง combinatorics — ครบทุกแบบต้อง ~24k+ ตัว ไม่คุ้ม).
**ผล (ไล่ลำดับ):** รอบแรก 648 → follow-up #1 (breeding 77-จว.) 1062 → **follow-up #2 (grid K=2) 5580 ตัว** (หมา 3234 = 77×21×2, แมว 2310 = 77×15×2, + breeding pre-like 6×real-pets). ตรวจ DB: province×breed **100%** (1617/1617, 1153/1155); จังหวัด+สายพันธุ์ที่เคยว่างทั้งหมดเจอแล้ว.

**Follow-up #4 (อายุ/นิสัย/ใบรับรองสุขภาพ ในทั้ง 2 โหมด):** combo เหล่านี้เคยเจอแต่ "บาง" (1–4 ตัว) เมื่อซ้อนบน province+breed เพราะ cell มีแค่ 2 ตัว. **แก้ `seed.ts`:** เพิ่ม `K_PER_CELL` 2→**4** + ทำให้ 4 ตัวในแต่ละ (province×breed) cell **กระจายแบบมีโครงสร้างตาม slot k**: เพศสลับ 2♂/2♀, ช่วงอายุตาม `AGE_BANDS` ([1-3],[4-6],[7-10],[2-8] → มีเด็ก/กลาง/แก่ทุก cell), สุขภาพหมุนตาม `(cell+k)` ไม่ผูกกับเพศ (vaccinated 3/4, neutered 2/4 → filter ฉีดวัคซีน/ทำหมันไม่ว่าง), tag primary หมุน global + extra 4–6. **ทุกตัวใน grid ถือ 2 โหมด** (`["playdate","breeding"]`) — เพราะแต่ละ cell มีตัวแทน age-band ละ 1 ตัว ถ้าทำ single-mode จะเจาะรู province+breed+age ของอีกโหมด (single-mode variety ย้ายไปอยู่ใน showcase 017 แทน). ลด realism เล็กน้อยแลกความเชื่อถือได้ของ filter. **ผล: ~11,124 ตัว** (หมา 6468=77×21×4, แมว 4620=77×15×4). ตรวจ DB ทั้ง 2 โหมด: breeding(golden♀) base 144/+neutered 63/+age7-10 13; playdate กทม+golden ทุก age band + neutered + tag+vaccinated ไม่ว่าง; province เล็ก (น่าน+ชิสุ) + age/neutered ก็ไม่ว่าง. *(combo ลึก 4–5 ชั้นยังเหลือ 1 ตัวได้แต่ไม่ว่าง.)*

**Follow-up #3 (empty-state copy):** เดิม deck ว่างขึ้น "ดูครบแล้ว!" เสมอ ซึ่งสื่อผิดตอนที่ว่างเพราะ "ตัวกรองแคบเกินไป" (ไม่ใช่ปัดครบ). แก้ `app/app/swipe/page.tsx`: แยก empty state ตาม `countActiveFilters(filters) > 0` — มีตัวกรอง → ไอคอน SlidersHorizontal + "ไม่พบสัตว์เลี้ยงที่ค้นหา / ไม่มีน้องที่ตรงกับตัวกรองนี้ ลองปรับหรือล้างตัวกรองดู" + ปุ่ม **ปรับตัวกรอง** (เปิด sheet) / **ล้างตัวกรอง** (`handleFilterChange(DEFAULT_FILTERS)`); ไม่มีตัวกรอง → คงเดิม "ดูครบแล้ว!" + "ดูใหม่อีกครั้ง". `npx tsc --noEmit` ผ่าน. (commit `4ac2a4d`) ตรวจ DB: ค่าที่เคยตาย→มีหมด (ไซบีเรียน ฮัสกี้ 14, เมนคูน 21, สงบเงียบ 118, ชอบออกกำลัง 128, เป็นมิตรกับสัตว์อื่น 123, น่าน 5, แม่ฮ่องสอน 4, คอมโบ ชลบุรี+ขี้เล่น 16). **ทุกบัญชี** (ไม่ใช่แค่ demo) เห็น pool นี้ เพราะ RLS pets = SELECT ทุกคน + swipe query ไม่ filter `owner_id` (กรองแค่ species+mode+ตัวกรอง). อัปเดต `CLAUDE.md` + `DEVLOG.md` (เลิกอ้าง "45 dogs + 40 cats"). `npx tsc --noEmit` (app + scripts) ผ่าน 0 error. **ไม่มี migration ใหม่** — งานนี้เป็น seed script ล้วน. *หมายเหตุ:* 017 demo-reset อิสระจากพูลนี้ (รีเฉพาะ 2 บัญชี demo).
