...
> อัปเดตล่าสุด: 2026-06-17 (session 18)
...
✅ Phase 8 — Lost Pet Board / ประกาศสัตว์หาย (Session 15) — feed + create form + detail + public share page พร้อม OG meta; public share page ยกระดับ UI ให้ตรง wireframe (Session 18)
...
## Session 18 — 2026-06-17 (Phase 8 Enhancement: Public Share Page)

### ยกระดับ `/lost/[id]` ให้ตรงกับ Wireframe

ย้อนกลับมาปรับ `/lost/[id]` (Public Share Page จาก Phase 8) ให้ตรงกับ `pawmate_wireframe/Public Lost Pet Share.dc.html` ทุก section

#### สิ่งที่เปลี่ยนแปลง

| ส่วน | เดิม | ใหม่ |
|---|---|---|
| บนสุด | Sticky amber login banner | PawMate logo bar (สีขาว + border) |
| รูปภาพ | static h-220 | h-240 + dot indicators เมื่อ photos.length > 1 |
| Celebration strip (found) | Text centered เท่านั้น | CheckCircle icon + 2-line layout (ซ้าย-ขวา) |
| Facts card | วันที่หาย + "(X วันที่ผ่านมา)" inline | แยก row "หายมาแล้ว X วัน" (Clock icon, สีแดง) สำหรับ lost / "กลับบ้านแล้ว" (CheckCircle, teal) สำหรับ found |
| Owner row | แสดงชื่อเจ้าของอย่างเดียว | เพิ่มปุ่ม "โทร" (`tel:` link) สำหรับ lost status เท่านั้น |
| Social proof | ไม่มี | Strip แสดงจำนวนเบาะแสจริง + "ช่วยกันแชร์" |
| CTA | ปุ่มธรรมดา "แจ้งเบาะแส (เข้าสู่ระบบ)" | CTA Banner Box ครบรูปแบบ: MapPin icon + heading + description + primary button + "สมัครสมาชิกฟรี" link |
| Share button | ไม่มี | `PublicShareButton` client component (Web Share API + clipboard fallback); lost = gray border, found = teal |
| Footer | Box border + ปุ่ม "สมัครฟรี" | Text-only logo + tagline; ปุ่ม "สมัครฟรี" ย้ายไป CTA banner แล้ว |
| OG description | "ช่วยตามหา {name} หายแถว..." | species · breed · distinguishing marks · หายมา X วัน · มีของรางวัล |

#### ไฟล์ที่สร้าง/แก้ไข

| ไฟล์ | สิ่งที่ทำ |
|---|---|
| `components/lost/PublicShareButton.tsx` | ใหม่ — client component, `variant="lost"` (gray border) / `variant="found"` (teal), ใช้ `window.location.href` (ไม่ต้อง hardcode URL) |
| `app/lost/[id]/page.tsx` | Rewrite UI ทั้งหมดตาม wireframe, เพิ่ม imports จาก lucide-react ทั้ง `Clock`, `CheckCircle`, `Phone`, `LogIn`, `Share2` |

#### Git & Push

```
feat(phase8): upgrade /lost/[id] public share page to match wireframe
# commit: 911cdde
